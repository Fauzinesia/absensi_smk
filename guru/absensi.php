...
$resGuru = db_query("SELECT g.guru_id, u.username FROM tb_guru g JOIN tb_users u ON g.user_id=u.user_id WHERE g.user_id = ? LIMIT 1", "i", [$userId]);
$guruId = 0; $username = '';
if ($resGuru instanceof mysqli_result && $resGuru->num_rows === 1) { $rowG = $resGuru->fetch_assoc(); $guruId = (int)$rowG['guru_id']; $username = (string)$rowG['username']; }
...
$resJ = db_query("SELECT jam_masuk, batas_telat, jam_pulang, is_libur FROM tb_jadwal_absensi WHERE berlaku_untuk='guru' AND hari=? LIMIT 1", "s", [$hari]);
...
<head>
    ...
    <title>Absensi Guru • E-Absensi</title>
...
<body>
                        ...
                        <div class="card absen-wrap">
                            <div class="card-body">
                                ...
                                <form id="form-absen" method="post" action="../admin/absensi/absensi.php" enctype="multipart/form-data">
                                    ...
                                    <input type="hidden" name="user_map" value="<?= (int)$guruId ?>">
    ...
    <script>
        ...
        fetch('../admin/absensi/absensi.php?action=status_today&jenis=guru&id=<?= (int)$guruId ?>').then(r=>r.json()).then(s=>{ HAS_MASUK=!!s.has_masuk; HAS_PULANG=!!s.has_pulang; refreshButtonState(); }).catch(()=>{});
                ...
                fetch('../admin/absensi/absensi.php',{method:'POST',headers:{'X-Requested-With':'XMLHttpRequest'},body:fd})
                    ...
                    .then(j=>{
                        if(j.status==='ok'){
                            ...
                            window.location.href='../guru/riwayat.php';
                        }else{
