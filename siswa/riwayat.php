...
$resSiswa = db_query("SELECT s.siswa_id, u.nama FROM tb_siswa s JOIN tb_users u ON s.user_id=u.user_id WHERE s.user_id = ? LIMIT 1", "i", [$userId]);
$siswaId = 0; $nama = 'Siswa';
if ($resSiswa instanceof mysqli_result && $resSiswa->num_rows === 1) { $rowS = $resSiswa->fetch_assoc(); $siswaId = (int)$rowS['siswa_id']; $nama = (string)$rowS['nama']; }
$from = isset($_GET['from']) && $_GET['from'] !== '' ? (string)$_GET['from'] : null;
$to = isset($_GET['to']) && $_GET['to'] !== '' ? (string)$_GET['to'] : null;
$q = "SELECT tanggal, status_masuk, jam_masuk, status_pulang, jam_pulang, jarak_masuk_meter FROM tb_absensi WHERE jenis='siswa' AND siswa_id = ? ";
$params = [$siswaId]; $types = "i";
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <div>
                                    <a href="../siswa/absensi.php" class="btn btn-primary"><i class="bi bi-camera"></i> Absen</a>
                                    <a href="../siswa/dashboard.php" class="btn btn-outline-secondary"><i class="bi bi-house"></i> Dashboard</a>
                                </div>
                            </div>
                            <div class="card-body">
