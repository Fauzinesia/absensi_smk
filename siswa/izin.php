...
$siswaId = 0; if ($resSiswa instanceof mysqli_result && $resSiswa->num_rows === 1) { $siswaId = (int)$resSiswa->fetch_assoc()['siswa_id']; }
$msg = null;
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($siswaId > 0 && in_array($jenis_izin, ['Izin','Sakit','Dinas','Dispensasi'], true) && $tanggal_mulai !== '' && $tanggal_selesai !== '') {
        $ok = db_query("INSERT INTO tb_izin (jenis_pengguna, siswa_id, tanggal_mulai, tanggal_selesai, jenis_izin, keterangan, bukti, status) VALUES ('siswa', ?, ?, ?, ?, ?, ?, 'Diajukan')", "isssss", [$siswaId, $tanggal_mulai, $tanggal_selesai, $jenis_izin, $keterangan !== '' ? $keterangan : null, $buktiPath]);
        $msg = $ok ? 'Izin diajukan' : 'Gagal mengajukan izin';
    } else {
        $msg = 'Isi data dengan benar';
    }
}
$riwayat = db_query("SELECT izin_id, tanggal_mulai, tanggal_selesai, jenis_izin, status, bukti FROM tb_izin WHERE jenis_pengguna='siswa' AND siswa_id = ? ORDER BY diajukan_pada DESC LIMIT 12", "i", [$siswaId]);
?>
<!DOCTYPE html>
<html lang="id">
<head>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <a href="../siswa/absensi.php" class="btn btn-outline-primary"><i class="bi bi-camera"></i> Absen</a>
                            </div>
                            <div class="card-body">
